...
OrbitalElementsDot computeDynamicsGVE(const OrbitalElements& oe, const ForceModelConfig& config, Real mu = constants::MU_EARTH);

DerivFunc makeTranslationalDeriv(const ForceModelConfig& config);
